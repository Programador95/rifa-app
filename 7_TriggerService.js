...
function limparDadosPreReservasExpiradas() {
  try {
    ...
    // Coletar todos os números expirados AGRUPADOS por código antes de salvar em Pendentes
    const gruposPorCodigo = {}; // { codigo: { nome, telefone, userId, numeros: [], timestamps: [], motivo } }
    ...
    for (let i = 1; i < valores.length; i++) {
      ...
      if (status === 'Pré-reservado') {
        ...
        if (tempoDecorrido > tempoLimite) {
          ...
          // Agrupar por código para salvar UMA linha por código no Pendentes
          if (codigo) {
            const chave = codigo.toString().trim();
            ...
            gruposPorCodigo[chave].numeros.push(numero.toString().padStart(3, '0'));
            // Usar o timestamp mais antigo do grupo
            if (timestampObj < gruposPorCodigo[chave].timestampReserva) {
              gruposPorCodigo[chave].timestampReserva = timestampObj;
            }
          }

          console.log(`🧹 Limpando número ${numero} (${minutosDecorridos} min) - Nome: ${nome}`);

          // Limpar colunas B, C, E, F, G, H, I
    ...
    // Salvar UMA linha por código no Pendentes (todos os números agrupados)
    Object.keys(gruposPorCodigo).forEach(function(codigo) {
      const grupo = gruposPorCodigo[codigo];
      try {
        salvarPendenteAgrupado({
          codigo: codigo,
          nome: grupo.nome,
          telefone: grupo.telefone,
          numeros: grupo.numeros,
          valor: grupo.numeros.length * VALOR_POR_NUMERO,
          timestampReserva: grupo.timestampReserva,
          motivo: grupo.motivo,
          userId: grupo.userId
        });
        pendentesSalvos++;
      } catch (erroPendente) {
        console.error(`⚠️ Erro ao salvar pendente código ${codigo}:`, erroPendente);
      }
    });
...
// ✅ Salvar pendente agrupado (múltiplos números de um mesmo código em UMA linha)
function salvarPendenteAgrupado(dados) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.appendRow([
        'Código WhatsApp', 'Nome', 'Telefone', 'Números', 'Valor (R$)',
        'Timestamp Reserva', 'Motivo Falha', 'Status', 'User ID', 'Data Registro', 'Data Recuperação'
      ]);
    ...
    const codigoStr = dados.codigo ? dados.codigo.toString() : '';

    // Se já existe um Pendente com este código, atualizar (evitar duplicata)
    const lastRow = sheet.getLastRow();
    if (lastRow >= 2) {
      const existentes = sheet.getRange('A2:H' + lastRow).getValues();
      for (let i = 0; i < existentes.length; i++) {
        const codEx = existentes[i][0] ? existentes[i][0].toString().trim() : '';
        const stEx = existentes[i][7] ? existentes[i][7].toString() : '';
        if (codEx === codigoStr && stEx === 'Pendente') {
          // Atualizar números (consolidar) e motivo
          const row = i + 2;
          ...
          console.log(`📝 Pendente agrupado atualizado: ${numerosStr} - Código: ${codigoStr}`);
          return;
        }
      }
    }
    ...
    sheet.appendRow([
      ...
      dados.motivo || 'Pré-reserva expirada',
      'Pendente',
      dados.userId || '',
      new Date(),
      ''
    ...
    sheet.getRange(newRow, 1).setNumberFormat('@').setValue(codigoStr);
    sheet.getRange(newRow, 4).setNumberFormat('@').setValue(numerosStr);
    console.log(`📝 Pendente agrupado criado: ${numerosStr} - Código: ${codigoStr}`);
  } catch (error) {
    console.error('❌ Erro ao salvar pendente agrupado:', error);
    throw error;
  }
}
