 ...
 * - ✅ Notificação via WhatsApp ao expirar reserva
 *
...
function limparDadosPreReservasExpiradas() {
  try {
    ...
    let linhasLimpas = 0;
    // ✅ NOVO: Agrupar números expirados por telefone para enviar 1 msg só
    const expiradosPorTelefone = {};
    ...
    for (let i = 1; i < valores.length; i++) {
      ...
      if (status === 'Pré-reservado') {
        ...
        if (tempoDecorrido > tempoLimite) {
          ...
          // ✅ NOVO: Capturar dados ANTES de limpar para notificação
          if (telefone && nome) {
            const telLimpo = telefone.toString().replace(/\D/g, '');
            if (telLimpo.length >= 10) {
              if (!expiradosPorTelefone[telLimpo]) {
                expiradosPorTelefone[telLimpo] = {
                  nome: nome,
                  telefone: telefone,
                  numeros: []
                };
              }
              expiradosPorTelefone[telLimpo].numeros.push(numero);
            }
          }
    ...
    // ✅ NOVO: Enviar notificações WhatsApp para cada telefone com reservas expiradas
    const telefonesNotificados = Object.keys(expiradosPorTelefone);
    if (telefonesNotificados.length > 0) {
      console.log(`📱 Enviando ${telefonesNotificados.length} notificação(ões) de expiração...`);
      telefonesNotificados.forEach(tel => {
        const dados = expiradosPorTelefone[tel];
        try {
          const resultado = enviarWhatsAppReservaExpirada(dados.nome, dados.telefone, dados.numeros);
          if (resultado.success) {
            console.log(`✅ Notificação enviada para ${dados.nome} (${dados.telefone}) - Números: ${dados.numeros.join(', ')}`);
          } else {
            console.log(`⚠️ Falha ao notificar ${dados.nome}: ${resultado.error}`);
          }
        } catch (e) {
          console.error(`❌ Erro ao notificar ${dados.nome}:`, e);
        }
      });
    }
    ...
    return {
      success: true,
      limpas: linhasLimpas,
      notificados: telefonesNotificados.length,
      timestamp: new Date(),
      message: `${linhasLimpas} números foram liberados, ${telefonesNotificados.length} notificação(ões) enviada(s)`
    };
  } catch (error) {
...
function configurarTudo() {
  ...
  try {
    ...
    console.log('- ✅ Trigger de limpeza a cada 5 minutos');
    ...
    console.log('- ✅ Notificação WhatsApp ao expirar reserva');
    console.log('');
