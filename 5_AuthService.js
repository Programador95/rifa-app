...
function buscarCodigoVerificacao(userId) {
    try {
        ...
        // Obter dados das colunas H (userId) e I (código) — nova estrutura v10
        const dados = sheet.getRange('H2:I' + lastRow).getValues();
  ...
  try {
    ...
    for (let i = 1; i < dados.length; i++) {
      ...
      if (numeroLinha === numeroStr) {
        ...
        const userIdLinha = linha[7] ? linha[7].toString().trim() : ''; // H - User ID
...
function debugUserId(userId) {
    try {
        ...
        if (lastRow >= 2) {
            const dados = sheet.getRange('H2:H' + lastRow).getValues();
