...
function reservarNumeros(dados) {
  try {
    ...
    const numerosRange = sheet.getRange('A2:I' + lastRow);
    ...
    dados.numeros.forEach(num => {
      ...
      for (let i = 0; i < todosOsDados.length; i++) {
        ...
        if (numeroPlanilha === numStr) {
          ...
          sheet.getRange(row, 2).setValue(dados.nome);      // B - Nome
          sheet.getRange(row, 3).setValue(dados.telefone);  // C - Telefone
          sheet.getRange(row, 4).setValue('Pré-reservado'); // D - Status
          // col 5 (E): Comprovante — vazio na pré-reserva
          // col 6 (F): Hash — vazio na pré-reserva (gravado só após comprovante aprovado)
          // ✅ Recovery mode: usar timestamp original da Pendentes para que OCR valide corretamente
          const tsParaGravar = dados.recoveryTimestamp ? new Date(dados.recoveryTimestamp) : new Date();
          sheet.getRange(row, 7).setValue(tsParaGravar);    // G - Timestamp
          sheet.getRange(row, 8).setValue(dados.userId);    // H - User ID
          sheet.getRange(row, 9).setValue(dados.codigo);    // I - Código
    ...
    // ✅ Enviar código PIX via WhatsApp (não bloqueia em caso de erro)
    if (dados.pixString && dados.telefone) {
      try {
        const todosNumsWA = dados.numeros.map(n => n.padStart(3, '0'));
        enviarCodigoPIXWhatsApp(
          dados.nome, dados.telefone, dados.pixString,
          todosNumsWA, dados.numeros.length * VALOR_POR_NUMERO, dados.codigo
        );
      } catch (erroWA) {
        console.warn('⚠️ Falha ao enviar PIX WA (não crítico):', erroWA);
      }
    }


    // ✅ NOVO: Salvar imediatamente na aba "Pendentes" para histórico de pré-reservas
    // Isso garante que, mesmo se o usuário sair da tela ou ocorrer qualquer problema
    // antes de enviar o comprovante, o registro já estará salvo para recuperação.
    try {
      // Agrupar números pelo mesmo código (todos terão o mesmo código aqui)
      const todosNumerosReservados = dados.numeros.map(n => n.padStart(3, '0'));

      // Verificar se já existe pendente com este código (evitar duplicata em retry)
      const ss = SpreadsheetApp.getActiveSpreadsheet();
      const pendentesSheet = ss.getSheetByName('Pendentes');
      let jaExiste = false;
      if (pendentesSheet && pendentesSheet.getLastRow() >= 2) {
        const pRows = pendentesSheet.getRange('A2:H' + pendentesSheet.getLastRow()).getValues();
        for (let pi = 0; pi < pRows.length; pi++) {
          if (pRows[pi][0].toString().trim() === dados.codigo.toString().trim() &&
              pRows[pi][7].toString() === 'Pendente') {
            jaExiste = true;
            break;
          }
        }
      ...
      if (!jaExiste) {
        // Salvar o primeiro número para criar o registro, os demais serão agrupados
        salvarPendente({
          numero: todosNumerosReservados[0],
          nome: dados.nome,
          telefone: dados.telefone,
          codigo: dados.codigo,
          valor: todosNumerosReservados.length * VALOR_POR_NUMERO,
          timestampReserva: new Date(),
          motivo: 'Pré-reserva iniciada (aguardando comprovante)',
          userId: dados.userId
        });

        // Adicionar demais números ao mesmo registro
        if (todosNumerosReservados.length > 1) {
          const sheetP = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Pendentes');
          if (sheetP) {
            const lastRowP = sheetP.getLastRow();
            const rowsP = sheetP.getRange('A2:H' + lastRowP).getValues();
            for (let pi = 0; pi < rowsP.length; pi++) {
              if (rowsP[pi][0].toString().trim() === dados.codigo.toString().trim() &&
                  rowsP[pi][7].toString() === 'Pendente') {
                const allNums = todosNumerosReservados.join(', ');
                sheetP.getRange(pi + 2, 4).setNumberFormat('@').setValue(allNums);
                sheetP.getRange(pi + 2, 5).setValue(todosNumerosReservados.length * VALOR_POR_NUMERO);
                break;
              }
            }
          }
        }
        console.log(`📝 Pré-reserva salva em Pendentes: ${todosNumerosReservados.join(', ')} - Código: ${dados.codigo}`);
      } else {
        console.log(`📝 Pendente já existe para código ${dados.codigo}, não duplicado`);
      }
    } catch (erroPendente) {
      // Não interrompe a reserva se falhar ao salvar em Pendentes
      console.error('⚠️ Erro ao salvar em Pendentes (não crítico):', erroPendente);
    }
...
function salvarComprovante(dados) {
  try {
    ...
    const lastRow = sheet.getLastRow();
    const linhas = sheet.getRange('A2:I' + lastRow).getValues();

    const numerosValidos = [];
    const numerosConflito = [];
    let codigoRecibo = ''; // Para capturar o código da coluna I
    ...
    numeros.forEach(numero => {
      ...
      for (let i = 0; i < linhas.length; i++) {
        ...
        if (numPlanilha === numStr) {
          ...
          const userIdPlanilha = linha[7]; // H - User ID (índice 7)
          const codigoLinha = linha[8];    // I - Código (índice 8)
          ...
          if (statusAtual === 'Pré-reservado' && userIdPlanilha === userId) {
            ...
            // Capturar código (coluna I, índice 8) para usar como recibo
    ...
    // Obter timestamp da reserva para validação OCR.
    // No fluxo normal: lê da coluna G (gravada no momento da pré-reserva).
    // No fluxo de recovery: usa recoveryTimestamp (timestamp original da Pendentes),
    // pois a coluna G agora armazena new Date() para evitar limpeza prematura pelo trigger.
    const primeiroNumero = numerosValidos[0];
    const timestampDaColG = sheet.getRange(primeiroNumero.linha + 2, 7).getValue();
    const timestampReserva = dados.recoveryTimestamp
      ? new Date(dados.recoveryTimestamp)
      : (timestampDaColG || new Date(Date.now() - 10 * 60 * 1000));
    ...
    if (!resultadoValidacao.valido) {
      ...
      // ✅ NOVO: Salvar/atualizar na aba "Pendentes" registrando o hash do comprovante inválido
      // Isso garante que o histórico da tentativa de comprovante esteja salvo,
      // e que o hash do comprovante rejeitado fique registrado para auditoria.
      try {
        salvarPendentePorFalhaComprovante({
          nome: nome,
          telefone: telefone,
          userId: userId,
          numeros: numerosValidos.map(n => String(n.numero).padStart(3, '0')),
          codigo: codigoRecibo || 'N/A',
          valor: numerosValidos.length * VALOR_POR_NUMERO,
          timestampReserva: timestampReserva,
          motivo: `Comprovante inválido: ${resultadoValidacao.motivo}`
        });
      } catch (erroPendente) {
        console.warn('⚠️ Erro ao atualizar Pendentes após falha OCR (não crítico):', erroPendente);
      }
    ...
    numerosValidos.forEach(({ numero, linha }) => {
      ...
      sheet.getRange(row, 2).setValue(nome);                             // B - Nome
      sheet.getRange(row, 3).setValue(telefone);                         // C - Telefone
      sheet.getRange(row, 4).setValue('Reservado');                      // D - Status
      sheet.getRange(row, 5).setValue(url);                              // E - Comprovante (link)
      sheet.getRange(row, 6).setValue(
        (resultadoValidacao.detalhes && resultadoValidacao.detalhes.hashComprovante)
          ? resultadoValidacao.detalhes.hashComprovante
          : ''
      ); // F - Hash Comprovante ✅
      sheet.getRange(row, 7).setValue(new Date());                       // G - Timestamp
      sheet.getRange(row, 8).setValue(userId);                           // H - User ID
      // col 9 (I): Código — manter o original
    ...
    // ✅ NOVO: Marcar o pendente como recuperado e salvar o hash do comprovante válido
    try {
      marcarPendenteComoRecuperado({ codigo: codigoRecibo });
    } catch (erroPendente) {
      console.warn('⚠️ Erro ao marcar pendente como recuperado (não crítico):', erroPendente);
    }
...
function cancelarReserva(dados) {
  try {
    ...
    // A: Número(0) | B: Nome(1) | C: Telefone(2) | D: Status(3) | E: Comprovante(4)
    // F: Hash(5) | G: Timestamp(6) | H: User ID(7) | I: Código(8)
    const range = sheet.getRange('A2:I' + lastRow);
    ...
    dados.numeros.forEach(num => {
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        if (numeroPlanilha === numStr) {
          ...
          const row = i + 2;
          const statusAtual = linha[3];
          const userIdAtual = linha[7]; // H - User ID (índice 7)
          ...
          if (podeSerCancelado) {
            ...
            sheet.getRange(row, 2).setValue(''); // B - Nome
            sheet.getRange(row, 3).setValue(''); // C - Telefone
            sheet.getRange(row, 4).setValue('Disponível'); // D - Status
            sheet.getRange(row, 5).setValue(''); // E - Comprovante
            sheet.getRange(row, 6).setValue(''); // F - Hash Comprovante
            sheet.getRange(row, 7).setValue(''); // G - Timestamp
            sheet.getRange(row, 8).setValue(''); // H - User ID
            sheet.getRange(row, 9).setValue(''); // I - Código
    ...
    if (numerosProcessados > 0) {
      ...
      // ✅ Atualizar motivo na aba Pendentes para refletir que o usuário saiu da tela
      // Mantemos status 'Pendente' para que recovery ainda funcione (pessoa pode ter pago)
      if (dados.codigo) {
        try {
          atualizarMotivoPendente(dados.codigo, 'Saiu da tela de pagamento (reserva cancelada pelo sistema)');
        } catch (erroPendente) {
          console.warn('⚠️ Não foi possível atualizar motivo no Pendentes (não crítico):', erroPendente);
        }
      }
    }
...
// =====================================================================
// ✅ POLLING: Verificar se comprovante via WhatsApp já foi processado
//
// Retorna:
//   { status: 'aguardando' }                     — nenhum evento ainda
//   { status: 'recebido' }                        — imagem recebida, OCR em andamento
//   { status: 'valido', numeros, codigoRecibo }   — aprovado
//   { status: 'invalido', motivo }                — rejeitado
// =====================================================================
function verificarStatusComprovanteWH(dados) {
  try {
    const { telefone, userId } = dados;
    const sheet   = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Rifa');
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) return { status: 'aguardando' };

    const telNorm = telefone.replace(/\D/g, '');

    const rows = sheet.getRange('A2:I' + lastRow).getValues();

    // Procurar por linha com esse telefone e status Reservado (comprovante aprovado)
    const numerosAprovados = [];
    let codigoRecibo = '';

    for (let i = 0; i < rows.length; i++) {
      const row    = rows[i];
      const status = (row[3] || '').toString().trim();
      const tel    = (row[2] || '').toString().replace(/\D/g, '');
      const uid    = (row[7] || '').toString().trim();

      // Verifica pelo telefone OU userId (para cobrir ambos os fluxos)
      const correspondeTelefone = tel.endsWith(telNorm) || telNorm.endsWith(tel);
      const correspondeUserId   = uid === userId;

      if ((correspondeTelefone || correspondeUserId) && status === 'Reservado') {
        const comprovante = (row[4] || '').toString().trim();
        // Só contar se o comprovante foi gerado nos últimos 15 minutos
        // (evitar confundir com compras antigas do mesmo telefone)
        const ts = row[6] ? new Date(row[6]) : null;
        const recente = ts && (Date.now() - ts.getTime()) < 15 * 60 * 1000;

        if (comprovante && recente) {
          numerosAprovados.push(row[0].toString().trim().padStart(3, '0'));
          if (!codigoRecibo && row[8]) codigoRecibo = row[8].toString().trim();
        }
      }
    }

    if (numerosAprovados.length > 0) {
      return { status: 'valido', numeros: numerosAprovados, codigoRecibo: codigoRecibo };
    }

    // Verificar se há status intermediário gravado pelo webhook (coluna J = índice 9)
    // O webhook grava 'wh_recebido' ou 'wh_processando' para sinalizar etapas
    for (let i = 0; i < rows.length; i++) {
      const row    = rows[i];
      const status = (row[3] || '').toString().trim();
      const tel    = (row[2] || '').toString().replace(/\D/g, '');
      ...
      if ((correspondeTelefone || correspondeUserId) && status === 'Pré-reservado') {
        // Verificar coluna J (índice 9) para status intermediário do webhook
        const statusWH = sheet.getRange(i + 2, 10).getValue();
        if (statusWH === 'wh_processando') return { status: 'verificando' };
        if (statusWH === 'wh_recebido')    return { status: 'recebido' };
        if (statusWH === 'wh_invalido') {
          const motivo = sheet.getRange(i + 2, 11).getValue() || 'Comprovante não reconhecido';
          return { status: 'invalido', motivo: motivo };
        }
      }
    }
    ...
    console.error('❌ verificarStatusComprovanteWH:', err.message);
    return { status: 'aguardando' };
  }
}
